    ...
    <div class="max-w-2xl mx-auto">
        ...
        @else
            ...
            @if ($kendaraans->isEmpty())
                <div class="text-center bg-white p-10 rounded-[2.5rem] border-2 border-dashed border-slate-200 shadow-sm">
                    <div
                    ...
                    <a href=""
                        class="mt-8 inline-flex items-center justify-center px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all uppercase tracking-widest">
                        <i class="fas fa-plus mr-2"></i>
                        Daftarkan Kendaraan
                    </a>
                </div>
            @else
                {{-- LIST KENDARAAN JIKA ADA --}}
                    ...
                    @foreach ($kendaraans as $k)
                        <a href="{{ route('mahasiswa.scanner', $k->id_kendaraan) }}"
                            ...
                            <div class="ml-5 flex-grow">
                                <p class="text-lg font-black text-slate-900 tracking-tight leading-none">
                                    {{ $k->nomor_polisi }}</p>
